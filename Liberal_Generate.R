...
lib_cor_matrix <- cor(cbind(predictors, outcomes), use = "pairwise.complete.obs")

# setting correlations that need to be set to 0 to 0 (among self, predictors --> predictors, etc.)
lib_cor_matrix[1:ncol(predictors), 1:ncol(predictors)] <- 0
lib_cor_matrix[1:ncol(outcomes), 1:ncol(outcomes)] <- 0
diag(lib_cor_matrix) <- 0

# using qgraph to make our network yay
qgraph(lib_cor_matrix, layout = "spring", weighted = TRUE, labels = colnames(cor_matrix), title = "Liberal Justices Correlations")
